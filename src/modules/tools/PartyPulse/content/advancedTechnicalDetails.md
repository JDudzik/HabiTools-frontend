...
- Because of how Habitica keeps track of log-ins and how Party Pulse checks this data, sometimes a member's score will jump up or down more than expected, but it will even-out the next day.
- Because loot-drops are technically random, we validate for 48hrs instead of 24hrs. This is to compensate for the fact that occasionally a member may have been active, but may not have received an item drop. This also means if a member received a drop, it will increase their score over two days instead of just one.